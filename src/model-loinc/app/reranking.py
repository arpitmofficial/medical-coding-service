...
async def rerank_codes(
    ...
    if content is None:
        fallback = sorted(candidates, key=lambda c: c.get("score", 0.0), reverse=True)[
            :FINAL_TOP_N
        ]
        return [
            {
    ...
    for rank, item in enumerate(parsed[:FINAL_TOP_N], start=1):
        ...
        confidence = int(
            max(0, min(100, (0.55 * base + 0.30 * llm_rank_score + 0.15 * facet) * 100))
        )
        ...
        reranked.append(
            {
                ...
                or "Ranked by semantic and facet match.",
            }
        )
    ...
    console_logger.info(
        f"LOINC reranking complete: returned {len(reranked[:FINAL_TOP_N])} codes"
    )
