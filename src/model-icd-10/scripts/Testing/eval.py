...
async def evaluate_single(diagnosis: str, expected_codes: set[str]) -> dict[str, Any]:
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        ...
        try:
            ...
            if _should_stop_on_llm_error(preprocessing_error):
                raise StopEvaluation(preprocessing_error or "LLM rate-limited / high traffic")
    ...
    raise RateLimitExhausted(str(last_error) if last_error else "rate limit retries exhausted")
...
async def run_eval():
    ...
    if RESULTS_PATH.exists():
        try:
            ...
            logger.info(f" Resuming: found {len(existing_results)} existing results. Skipping {len(tested_diagnoses)} tested diagnoses.")
        except Exception as e:
    ...
    diagnosis_col = next((c for c in df.columns if "diagnosis" in c.lower()), df.columns[0])
    codes_col = next((c for c in df.columns if "code" in c.lower()), df.columns[1])
    
    logger.info(f"Using diagnosis column: '{diagnosis_col}', codes column: '{codes_col}'")
    ...
    if results:
        ...
        logger.info(f" Results saved to {RESULTS_PATH} ({len(results)} total, {len(results) - len(existing_results)} new)")
        ...
        if stopped_early:
            print("  NOTE: Stopped early due to rate limiting; rerun to resume from the next untested row.")
        print("="*100 + "\n")
    else:
