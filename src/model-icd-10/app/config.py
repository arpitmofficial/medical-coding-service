...
QDRANT_TOP_K: int = int(
    os.getenv("QDRANT_TOP_K", "50")
)  # candidates per entity query (Qdrant fetch limit)
PRE_RERANK_TOP_N: int = int(
    os.getenv("LOINC_PRE_RERANK_TOP_N", "50")
)  # max pool size after fusion
# before re-rank/output
FINAL_TOP_N: int = int(
    os.getenv("FINAL_TOP_N", "5")
)  # final codes returned to the caller
MIN_SCORE: float = float(
    os.getenv("MIN_SCORE", "0.0")
)  # minimum fused score to enter the pool (0 = off)
