...
#  COLUMN DETECTION
# =========================
CODE_CANDIDATES = ["CPT", "CPT Code", "CPT Codes", "Code", "Procedure Code"]
...
CATEGORY_CANDIDATES = ["Procedure Code Category", "Category", "Section"]
...
async def setup(client, collection, recreate):
    ...
    res = await client.put(
        f"{QDRANT_URL}/collections/{collection}", headers=QDRANT_HEADERS, json=schema
    )
...
async def ingest(path, collection, batch_size, start, recreate):
    ...
    async with httpx.AsyncClient(timeout=120.0) as client:
        ...
        for i in range(start, total, batch_size):
            ...
            texts = [f"CPT {codes[j]} {cats[j]} {descs[j]}" for j in range(len(codes))]
